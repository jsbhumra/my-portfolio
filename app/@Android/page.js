...
export default function Android() {
  ...
  return (
    <div className='w-full h-[100dvh] overflow-hidden bg-[#f0f2f5] dark:bg-[#111b20]'>
      {currChat ? 
      infoOpen ? 
        <GroupInfo onHide={()=>{setInfoOpen(false)}} anim={infoOpen} curr={currChat} theme={theme} />
      :
      <div id='right' className='relative w-full h-full overflow-hidden grow origin-top-left'>
        <ChatSection curr={currChat} onHide={()=>{setCurrChat()}} onShow={()=>{setInfoOpen(true)}} theme={theme} />
      </div> :
      <div className='w-full h-[100dvh] bg-transparent flex flex-col'>
        <div className='grow-0'>
          <div id="topHeader" className='w-full h-[59px] bg-[#028069] dark:bg-[#1f2c33] flex justify-between items-center pl-[20px] py-0 my-0 text-white dark:text-[#dadee0]'>
          ...
          //   indicatorColor="secondary"
            ...
            TabIndicatorProps={{
              style: {
                backgroundColor: theme=="dark" ? "#0ea783" : "#ffffff",
                height: '4px'
              },
            }}
            variant="fullWidth"
            aria-label="full width tabs example"
            sx={{ backgroundColor: theme=="dark" ? '#1f2c33' : "#028069", height: '59px', marginTop: '-15px', alignItems: 'flex-end', padding: 0, border: 0, "& button.Mui-selected": { color: theme=="dark" ? "#0ea783" : "#ffffff" } }}
          >
  ...
  </svg>} value={0} sx={{ maxWidth: '16%', minWidth: '16%', color: theme=="dark" ? '#6b7175' : "rgba(255,255,255,0.4)", fontWeight: 900 }} />
            <Tab value={1} label="Chats" sx={{maxWidth: '28%', minWidth: '28%', textTransform: 'capitalize', color: theme=="dark" ? '#6b7175' : "rgba(255,255,255,0.4)", letterSpacing: '0.06em', fontWeight: 500, fontSize: '1rem' }} />
            <Tab value={2} label="Tech" sx={{maxWidth: '28%', minWidth: '28%', textTransform: 'capitalize', color: theme=="dark" ? '#6b7175' : "rgba(255,255,255,0.4)", letterSpacing: '0.06em', fontWeight: 500, fontSize: '1rem' }} />
            <Tab value={3} label="Profile" sx={{maxWidth: '28%', minWidth: '28%', textTransform: 'capitalize', color: theme=="dark" ? '#6b7175' : "rgba(255,255,255,0.4)", letterSpacing: '0.06em', fontWeight: 500, fontSize: '1rem' }} />
          </Tabs>
        </div>
        <SwipeableViews
          id='mainBody'
        ...
        >
          ...
          <TabPanel className='w-full h-[calc(100dvh-93px)]' value={value} index={1} dir={theme.direction}>
            {Chats.map((el)=>{
              ...
              return <ChatGroup key={el.id} name={el.name} datetime={el.datetime} text={ messFrom+lastMess } /*text={el.text}*/ img={el.img} mute={el.muted} onClick={()=>{setCurrChat(el.id);}} selected={currChat==el.id} theme={theme} />
            })}
          </TabPanel>
